...
export function QuinxPage() {
  ...
  return <main id="top" className="quinx-shell">
    ...
    <section className="services section-pad" id="services"><Reveal><p className="eyebrow">Capabilities</p><h2>What we do</h2></Reveal><ScrollStack className="quinx-capability-stack" itemDistance={72} itemScale={.025} itemStackDistance={28} stackPosition="20%" scaleEndPosition="10%" baseScale={.9} rotationAmount={0} blurAmount={0} useWindowScroll>{services.map(([number, group, title, description]) => <ScrollStackItem key={title} itemClassName="service-row"><span>{number}</span><h3>{group} <small>— {title}</small></h3><p>{description}</p><ArrowUpRight aria-hidden="true" /></ScrollStackItem>)}</ScrollStack></section>
    ...
    <section className="process section-pad"><Reveal><p className="eyebrow">Our process</p><h2>From idea to impact.</h2></Reveal><div className="process-grid">{process.map(([number, title, description], index) => <Reveal key={number} delay={index * .14}><article><span>{number}</span><h3>{title}</h3><p>{description}</p></article></Reveal>)}</div></section>

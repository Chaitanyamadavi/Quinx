...
export default function ScrollExpand({
  ...
  const applyProgress = useCallback((progress: number) => {
    ...
    if (titleRef.current) {
      const outgoing = smoothstep(0.4, 0.88, progress)
      titleRef.current.style.opacity = `${1 - outgoing}`
      titleRef.current.style.transform = `translate3d(0, ${-28 * outgoing}px, 0) scale(${1 + 0.06 * outgoing})`
    }
    if (hintRef.current) {
  ...
  useEffect(() => {
    ...
    const readProgress = () => {
      ...
      const span = stageHeight * Math.max(0.01, currentProps.scrollDistance)
      if (currentProps.useWindowScroll) return clamp(-track.getBoundingClientRect().top / span, 0, 1)
      return clamp(root.scrollTop / span, 0, 1)
    }

    const tick = () => {
      const currentProps = propsRef.current
      const follow = currentProps.smoothing <= 0 ? 1 : 1 - Math.exp(-1 / (60 * currentProps.smoothing))
  ...
  return <div ref={rootRef} className={`scroll-expand ${useWindowScroll ? 'scroll-expand--window' : 'scroll-expand--scroller'} ${className}`.trim()} style={style}>
